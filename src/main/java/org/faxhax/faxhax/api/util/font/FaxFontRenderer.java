...
public class FaxFontRenderer extends FaxFont {
    ...
    public FaxFontRenderer(Font font, boolean antiAlias, boolean fractionalMetrics) {
    ...
    public float drawStringWithShadow(String text, double x, double y, FaxColor color) {
        float shadowWidth = drawString(text, x + 1D, y + 1D, color, true);
        return Math.max(shadowWidth, drawString(text, x, y, color, false));
    }

    public float drawString(String text, float x, float y, FaxColor color) {
        return drawString(text, x, y, color, false);
    }

    public float drawCenteredStringWithShadow(String text, float x, float y, FaxColor color) {
        return drawStringWithShadow(text, x - getStringWidth(text) / 2, y, color);
    }

    public float drawCenteredStringWithShadow(String text, Rectangle rectangle, FaxColor color) {
        return drawStringWithShadow(text, (float) (rectangle.getCenterX() - (getStringWidth(text) / 2)), (float) (rectangle.y + (rectangle.height / 4)), color);
    }

    public float drawCenteredString(String text, float x, float y, FaxColor color) {
        return drawString(text, x - getStringWidth(text) / 2, y, color);
    }

    public float drawString(String text, double x, double y, FaxColor gsColor, boolean shadow) {
        x -= 1;
        y -= 2;
        FaxColor color=new FaxColor(gsColor);
        if (text == null) return 0.0F;
        if (color.getRed()==255 && color.getGreen()==255 && color.getBlue()==255 && color.getAlpha()==32) color=new FaxColor(255,255,255);
        if (color.getAlpha()<4) color=new FaxColor(color,255);
        if (shadow) color=new FaxColor(color.getRed()/4,color.getGreen()/4,color.getBlue()/4,color.getAlpha());
