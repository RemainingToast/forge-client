...
public class FaxEntityUtil {
    ...
    public static void attackEntity(final Entity entity, final boolean packet, String handMode) {
        if (packet) {
        ...
        if(handMode.equalsIgnoreCase("MainHand")) swingArm(EnumHand.MAIN_HAND);
        if(handMode.equalsIgnoreCase("OffHand")) swingArm(EnumHand.OFF_HAND);
        if(handMode.equalsIgnoreCase("Both")) {
    ...
    {
        return getInterpolatedPos(entity, ticks).subtract(mc.getRenderManager().viewerPosX,
